...
public class staffController {
    ...
    public static boolean insertStaff(String name, String email, String password, int contactNo, String image) {
        try {
            con = DBConnection.getConnection();
            stmt = con.createStatement();

            String sql = "INSERT INTO staff (name, email, password, contactNo, image) VALUES ('"
                    + name + "', '" + email + "', '" + password + "', '" + contactNo + "', '" + image + "')";
    ...
    public static List<staffModel> loginValidate(String email, String password) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                staffModel staff = new staffModel(id, name, email, pass, contactNo, image);
                staffList.add(staff);
            }
    ...
    public static List<staffModel> getStaffById(String id) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                staffModel staff = new staffModel(sid, name, email, password, contactNo, image);
                staffList.add(staff);
            }
...
//try {
  //  con = DBConnection.getConnection();
    ...
    // SQL query to delete the staff by id
    //String sql = "DELETE FROM staff WHERE id='" + id + "'";
    
    //int result = stmt.executeUpdate(sql);
    //isSuccess = result > 0; // Return true if deletion was successful
